...
public final class WindowGL implements IWindow, IEngineComponent, IThreadable
{
    ...
    /*private void initializeProperties() {
        Property propPosition = new Property(WindowProperties.WINDOW_POSITION, new Point(64, 64));
        Property propSize = new Property(WindowProperties.WINDOW_SIZE, new Point(640, 480));
        //Property propMonitorWidth;
        //Property propMonitorHeight;
        Property propFPS = new Property(WindowProperties.FPS, (long) 0);
        Property propTitle = new Property(WindowProperties.TITLE, "Powered by JOHNEngine v.1.0.0");
        Property propHasBorder = new Property(WindowProperties.HAS_BORDER, true);
        Property propLockCursorToCenter = new Property(WindowProperties.LOCK_CURSOR_TO_CENTER, false);
        Property propIsCursorVisible = new Property(WindowProperties.IS_CURSOR_VISIBLE, true);
        Property propWindowState = new Property(WindowProperties.WINDOW_STATE, STATE_INITIALIZING);
        Property propUseVSync = new Property(WindowProperties.USE_V_SYNC, false);
        Property propIsFocused = new Property(WindowProperties.IS_FOCUSED, true);
        Property propIsMaximized = new Property(WindowProperties.IS_MAXIMIZED, false);
        Property propIsFullscreen = new Property(WindowProperties.IS_FULLSCREEN, false);
        
        propPosition.setAction((Object value) -> {
            Point wpos = (Point) value;
            GLFW.glfwSetWindowPos(windowID, wpos.x, wpos.y);
        });
        
        propSize.setAction((Object value) -> {
            Point size = (Point) value;
            GLFW.glfwSetWindowSize(windowID, size.x, size.y);
        });
        
        propTitle.setAction((Object title) -> {
            GLFW.glfwSetWindowTitle(windowID, (String) title);
        });
        
        propIsCursorVisible.setAction((Object isVisible) -> {
            DebugUtils.log("lol", isVisible);
            GLFW.glfwSetInputMode(
                windowID,
                GLFW.GLFW_CURSOR,
                (boolean) isVisible ? GLFW.GLFW_CURSOR_NORMAL : GLFW.GLFW_CURSOR_HIDDEN
            );
        });
        
        propUseVSync.setAction((Object useVSync) -> {
            GLFW.glfwSwapInterval((boolean) useVSync ? 1 : 0);
        });
        
        propIsFullscreen.setAction((Object useVSync) -> {
            GLFWVidMode videoMode = GLFW.glfwGetVideoMode(primaryMonitorID);
            properties.getProperty(WindowProperties.WINDOW_SIZE)
            .setCurrent(new Point(videoMode.width(), videoMode.height()));
            
            rebuildWindow();
        });
        
        this.setProperty(propPosition);
        this.setProperty(propSize);
        //this.setProperty(propMonitorWidth);
        //this.setProperty(propMonitorHeight);
        this.setProperty(propFPS);
        this.setProperty(propTitle);
        this.setProperty(propHasBorder);
        this.setProperty(propLockCursorToCenter);
        this.setProperty(propIsCursorVisible);
        this.setProperty(propWindowState);
        this.setProperty(propUseVSync);
        this.setProperty(propIsFocused);
        this.setProperty(propIsMaximized);
        this.setProperty(propIsFullscreen);
    }
    ...
    public void start() {
        ...
        GLFW.glfwMakeContextCurrent(this.windowID);
        this.renderer.initialize();
        this.properties.getProperty(WindowProperties.WINDOW_STATE).setCurrent(STATE_OPEN);
    ...
    public void loop() {
        ...
        while( !this.isWindowClosing() )
        {
            ...
            if( this.isCursorLockedToCenter() && previousInputTimestamp != inputTimestamp )
            {
                Point size = (Point) this.properties.getProperty(WindowProperties.WINDOW_SIZE).getCurrent();
                GLFW.glfwSetCursorPos(this.windowID, size.x / 2, size.y / 2);
            }
            ...
            if( currentTime - startTime >= 1000 )
            {
                this.properties.getProperty(WindowProperties.FPS).setCurrent(fpsCounter);
                fpsCounter = 0;
                startTime = currentTime;
            }
        }
    ...
    public void stop() {
        ...
        this.properties.getProperty(WindowProperties.WINDOW_STATE).setCurrent(STATE_CLOSED);
    }
    ...
    protected long createWindow() {
        ...
        GLFW.glfwWindowHint(
            GLFW.GLFW_DECORATED,
            (boolean) this.properties.getProperty(WindowProperties.IS_FULLSCREEN).getCurrent() ? 
            GLFW.GLFW_FALSE : 
            GLFW.GLFW_TRUE
        );
        ...
        long winID = GLFW.glfwCreateWindow(
            size.x,
            size.y,
            (String) this.properties.getProperty(WindowProperties.TITLE).getCurrent(),
            MemoryUtil.NULL,
            MemoryUtil.NULL
        );
        ...
        GLFW.glfwSetInputMode(winID, GLFW.GLFW_CURSOR,
            (boolean) this.properties.getProperty(WindowProperties.IS_CURSOR_VISIBLE).getCurrent() ?
            GLFW.GLFW_CURSOR_NORMAL : 
            GLFW.GLFW_CURSOR_HIDDEN
        );
        
            // Position the window
        Point wpos = (Point) this.properties.getProperty(WindowProperties.WINDOW_POSITION).getCurrent();
        GLFW.glfwSetWindowPos(winID, wpos.x, wpos.y);
    ...
    void rebuildWindow() {
        GLFW.glfwDestroyWindow(this.windowID);
        this.properties.getProperty(WindowProperties.WINDOW_STATE).setCurrent(STATE_INITIALIZING);
        this.windowID = this.createWindow();
        this.properties.getProperty(WindowProperties.WINDOW_STATE).setCurrent(STATE_OPEN);
        GLFW.glfwMakeContextCurrent(this.windowID);
    }*/
    ...
    /*protected void focusListener(boolean isFocused) {
        this.properties.getProperty(WindowProperties.IS_FOCUSED).setCurrent(isFocused);
    }
    
    protected void maximizeListener(boolean isMaximized) {
        this.properties.getProperty(WindowProperties.IS_MAXIMIZED).setCurrent(isMaximized);
    }
    
    protected void positionListener(int xpos, int ypos) {
        this.properties.getProperty(WindowProperties.WINDOW_POSITION).setCurrent(new Point(xpos, ypos));
    }
    
    protected void resizeListener(int width, int height) {
        this.properties.getProperty(WindowProperties.WINDOW_SIZE).setCurrent(new Point(width, height));
    }
